angular.module('cs411', ['ngRoute', 'ngCookies'])
    ...
    .controller('cs411ctrl', function ($scope, $http, $cookies) {
        ...
        $scope.present_weather = function (input, name) {
        ...
        $scope.present_title = function (input) {
            $http.get('http://localhost:3000/googlesearch/' + input)
                .then(function (response) {
                    $scope.result1 = response.data.items
                    $scope.key = input
                })
        }
